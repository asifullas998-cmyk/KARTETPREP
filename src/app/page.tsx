    ...
    <div className="flex flex-col items-center justify-center space-y-12">
      <section className="text-center">
        <h1 className="font-headline text-4xl md:text-5xl lg:text-6xl font-bold tracking-tight text-primary">
          Welcome to KARTET Prep
        </h1>
        <p className="mt-4 max-w-2xl text-lg text-muted-foreground">
          Your one-stop solution for acing the Karnataka Teacher Eligibility Test. Practice questions, mock tests, and personalized study plans, all in one place.
        </p>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-5xl">
        <FeatureCard
          ...
          description="Sharpen your skills with our extensive, multilingual question bank covering all subjects."
        />
        <FeatureCard
          href="/mock-tests"
          icon={<FileText className="w-8 h-8 text-accent" />}
          title="Mock Tests"
          description="Simulate the real exam experience with full-length, timed mock tests for Paper I & II."
        />
        <FeatureCard
          href="/syllabus"
          icon={<BookOpen className="w-8 h-8 text-accent" />}
          title="Syllabus"
          description="Access the complete, structured KARTET syllabus to guide your preparation effectively."
        />
      </section>
    </div>
  );
